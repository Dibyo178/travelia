...
const Hero = () => {
  ...
  // --- ডাটাবেসের সাথে মেম্বার ডাটা সিঙ্ক করার ফাংশন ---
  const syncUserWithDatabase = async (firebaseUser, provider) => {
    const userData = {
      uid: firebaseUser.uid,
      name: firebaseUser.displayName || fullName,
      email: firebaseUser.email,
      provider: provider
    };

    try {
      // এখানে memberRoutes এর এন্ডপয়েন্ট ব্যবহার করা হয়েছে
      await axios.post(`${BASE_URL}/api/members/sync-member`, userData);
  ...
  const handleEmailAuth = async () => {
    ...
    try {
      if (isRegister) {
        ...
        await syncUserWithDatabase(userCredential.user, 'email');
  ...
  return (
    <section className="px-4 lg:px-10 py-4 lg:py-6">
      ...
      <div className="relative bg-[#0b0d10] rounded-[30px] lg:rounded-[40px] min-h-[550px] lg:min-h-[600px] flex flex-col lg:flex-row overflow-hidden border border-white/5 shadow-2xl">
        ...
        <div className="w-full lg:w-[50%] p-6 lg:p-24 flex flex-col justify-center z-20 relative order-2 lg:order-1">
          ...
          <div className="flex flex-wrap items-center gap-4 lg:gap-6">
            {user ? (
              <div className="flex items-center gap-3 bg-white/5 p-1 pr-4 rounded-full border border-white/10 group">
                {/* প্রোফাইল বাটনে ক্লিক করলে /profile এ যাবে */}
                <div
                  onClick={() => navigate('/profile')}
                  className="w-10 h-10 bg-amber-400 rounded-full flex items-center justify-center text-black font-black shadow-lg cursor-pointer hover:scale-105 transition-all"
                >
                  {user.name?.charAt(0).toUpperCase()}
                </div>
                <div className="flex flex-col">
                  <span onClick={() => navigate('/profile')} className="text-white text-xs font-bold cursor-pointer hover:text-amber-400">{user.name?.split(' ')[0]}</span>
                  <button onClick={handleLogout} className="text-[10px] text-red-500 font-black uppercase tracking-tighter hover:text-white text-left transition-all">Logout</button>
                </div>
              </div>
            ) : (
              // লগইন না থাকলে সাইন-ইন বাটন দেখাবে
              <>
                <div
      ...
      {/* Auth Modals (বাকি কোড আপনার আগের মতোই অপরিবর্তিত আছে) */}
      {showAuthModal && (
            ...
            <div className="space-y-4">
              {isRegister && (
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 p-4 rounded-xl text-white outline-none focus:border-amber-400 transition-all"
                />
              )}
              <input
                type="email"
              ...
              <button
                ...
                className="w-full bg-amber-400 text-black font-black py-4 rounded-xl hover:bg-white transition-all shadow-lg active:scale-95"
              >
