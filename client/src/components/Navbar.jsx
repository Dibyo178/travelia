...
const Navbar = () => {
  ...
  const [allTours, setAllTours] = useState([]); // ডাটাবেস থেকে আসা সব ট্যুর
  const [searchResults, setSearchResults] = useState([]);
  const [showSearch, setShowSearch] = useState(false);
  const navigate = useNavigate();
  const searchRef = useRef(null);

  // ১. ডাটাবেস থেকে ডাইনামিক ডাটা ফেচ করা
  useEffect(() => {
  ...
  // ২. সার্চ লজিক: Title এবং Location উভয় দিয়ে ফিল্টার হবে
  useEffect(() => {
    if (searchTerm.trim()) {
    ...
    <nav className="bg-white sticky top-0 z-[100] shadow-sm">
      <div className="flex items-center justify-between px-6 lg:px-12 py-5 max-w-7xl mx-auto relative">
        ...
        <ul className="hidden lg:flex items-center gap-8 font-bold text-slate-700 uppercase text-[12px] tracking-widest">
          {navLinks.map((link) => (
            <li key={link.name}>
              <NavLink to={link.path} className={({ isActive }) => `transition-colors duration-300 ${isActive ? 'text-orange-500 border-b-2 border-orange-500 pb-1' : 'hover:text-orange-500'}`}>
                {link.name}
              </NavLink>
            </li>
          ))}
          ...
          <div className="relative flex items-center">
            {showSearch && (
              <div className="absolute right-12 flex flex-col items-end">
                ...
                {/* ডাইনামিক সার্চ রেজাল্ট ড্রপডাউন */}
                    ...
                    <div className="max-h-[300px] overflow-y-auto custom-scrollbar">
                      {searchResults.map((tour) => (
                        <div
                          key={tour.id}
                          onClick={() => {
                            navigate(`/package/${tour.id}`); // সরাসরি সেই প্যাকেজে নিয়ে যাবে
                            setShowSearch(false);
                            setSearchTerm("");
                          }}
            ...
            <button onClick={() => setShowSearch(!showSearch)} className={`p-3 rounded-full transition duration-300 ${showSearch ? 'bg-orange-500 text-white' : 'bg-gray-100 text-slate-700 hover:bg-orange-100'}`}>
              <FaSearch />
            </button>
          </div>
          
          <button onClick={() => navigate('/tours')} className="hidden md:block bg-black text-white px-7 py-3 rounded-full font-black text-[11px] uppercase tracking-widest hover:bg-orange-600 transition duration-300 shadow-lg">
            Explore Trip
          </button>
        </div>
      </div>
